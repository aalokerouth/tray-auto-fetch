...
# 1. Fix the PyInstaller "--windowed" crash by creating dummy console outputs
if sys.stdout is None:
    sys.stdout = open(os.devnull, "w")
if sys.stderr is None:
    sys.stderr = open(os.devnull, "w")

# 2. Tell Playwright to use local browsers BEFORE importing playwright
os.environ["PLAYWRIGHT_BROWSERS_PATH"] = "0"

# 3. NOW you can safely import playwright and other modules
from playwright.sync_api import sync_playwright
from datetime import datetime
from datetime import timedelta

# -- Your setup logic continues --
import json

# 1. Figure out where the .exe is actually running from
if getattr(sys, 'frozen', False):
    BASE_DIR = os.path.dirname(sys.executable)
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. Set a default download directory just in case
DOWNLOAD_DIR = BASE_DIR 

# 3. Magically read the folder selected by main.py from config.json
config_path = os.path.join(BASE_DIR, "config.json")
try:
    if os.path.exists(config_path):
        with open(config_path, "r", encoding="utf-8") as f:
            config_data = json.load(f)
            # If main.py saved a folder, use it!
            if "data_folder" in config_data and config_data["data_folder"]:
                DOWNLOAD_DIR = config_data["data_folder"]
